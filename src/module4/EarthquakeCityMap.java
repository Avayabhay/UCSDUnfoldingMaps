...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		// FOR TESTING: Set earthquakesURL to be one of the testing files by uGeoMapApp.TopologicalGeoMapProvider()ncommenting
...
//		ellipse(50, 225, 5, 5);
		...
		fill(color(255,69,0));
		ellipse(40,275,10,10);
		
		fill(0, 0, 0);
		text("City Marker", 65, 125);
		text("Ocean Earthquakes",60, 150);
		text("Land Earthquakes",60, 175);
		text("SIZE  ~~  MAGNITUDE",35,200);
		text("Shallow",60,225);
		text("Intermediate",60,250);
		text("Deep",60,275);
		//text()
	...
	private void printQuakes() 
	{
		...
		for(Marker c : countryMarkers){
			int quakeCount =0;
			oceanQuakes =0;
			String country = (String)c.getProperty("name");
			for(Marker m: quakeMarkers) {
				EarthquakeMarker mark = (EarthquakeMarker)m;
				if(mark.isOnLand) {
					String countryEarthquake = (String)mark.getProperty("country");
					
					if(country == countryEarthquake) {
						quakeCount++;
					}
				}
				else {
		...
		System.out.println("OceanQuakes Counts are - "+oceanQuakes);
